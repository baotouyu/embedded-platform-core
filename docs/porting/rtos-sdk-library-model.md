...
例如 D12x/Luban-Lite 当前会收集 `d12x.elf`、`d12x.bin`、`d12x.map` 和 Luban-Lite 生成的整包 `*.img`。烧录通常使用 SDK 生成的 `*.img`，调试链接问题时看 `<chip>.elf`、`<chip>.map` 和 `build.log`。
...
`ui.lvgl_provider` 用来声明当前 target 的 LVGL 来源：

| 值 | 含义 | 典型场景 |
| --- | --- | --- |
| `sdk` | LVGL 由目标 SDK 内置提供 | RTOS 原厂 SDK 已经带显示、触摸和 LVGL port，例如 Luban-Lite |
| `component` | LVGL 由主工程组件或独立组件仓库提供 | Linux 应用独立交叉编译，例如 F133 的 `sunxi_lvgl_v9.1` |
| `prebuilt` | LVGL 由主工程消费预编译包 | host/macOS 或已经产好的平台 LVGL 包 |
| `none` | 当前 target 不提供 LVGL | stub target 或无 UI 产品 |

RTOS 原厂 SDK 已经带 LVGL 时，不把 LVGL 源码复制到 `components/`，也不在 EP HAL 中二次封装 display/touch。主工程只保留 `components/ui` 这种生命周期薄封装和 target 能力声明，具体显示刷新、触摸输入、缓冲和加速继续由 SDK 的 LVGL port 负责。
...
这个命令只校验 `targets/*.yaml` 的结构和约定，不拉取 SDK，也不执行编译。它会检查 target 名、`platform:` 分组、SDK 来源字段、UI/LVGL 来源、输出目录和禁止的本地 SDK 路径。
...
```text
<workspace-parent>/sdk-artinchip-luban-lite
```
...
- **LVGL 来源声明**：D12x/Luban-Lite target 的 `ui.lvgl_provider=sdk`，表示显示、触摸和 LVGL port 由原厂 Luban-Lite SDK 提供，主工程不接管 LVGL 源码
...
下一阶段不再是“接入真实编译”或“补齐所有底层外设”。D12x/Luban-Lite/KI-141103-480p 已经完成基础真实平台适配：

- `check-env` / `install-env` 已作为 SDK 标准环境检测和安装入口落地。
- 工具链可从 Release 恢复到原厂 Luban-Lite `toolchain/` 位置。
- `build-firmware` 已调用真实 Luban-Lite SCons 构建。
- `app/main.c` 已通过 `libep_app_core.a` 链接进最终镜像。
- RT-Thread OSAL 已接入内存、时间、线程、mutex、sem、queue。
- RT-Thread HAL 已接入 UART、PWM、GPIO、I2C、RTC。
- KI-141103-480p 已完成基础烧录启动和主要板级冒烟。

当前主线建议转为业务应用结构整理：

1. 从 `app/main.c` 拆分业务模块和初始化顺序。
2. 电源板 UART2 协议按真实帧格式单独实现。
3. UI 页面直接使用当前芯片 LVGL port，不在 EP HAL 中再封 display/touch。
4. SD 卡文件系统使用 Luban-Lite/RT-Thread SDK 已有能力，业务需要时按平台 `open/read/write` 路线使用。
5. SPI、ADC 当前业务暂时不用，公共接口保留，真实 port 按需再补。

兼容层总览见 `docs/porting/luban-lite-compatibility-overview.md`，板级验证见 `docs/porting/ki-141103-480p-smoke-test.md`。
