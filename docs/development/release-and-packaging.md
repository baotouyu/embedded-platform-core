...
- 主工程不提交大型厂商 SDK。
- 厂商 SDK 放到外部 SDK 仓库管理。RTOS 平台由主工程导出应用核心静态库包，芯片 SDK 仓库负责链接、打包和烧录。
...
- `sdk-artinchip-luban-lite/` 维护官方 Luban-Lite SDK、`ep_app` 入口、defconfig、SDK 构建脚本和烧录脚本。
- `out/firmware/<target>/` 放 SDK 链接主工程静态库后生成的最终固件。
