...
- `app/ui/` 公共页面入口接入 host/macOS demo。
...
当前 UI 代码策略已经明确：Mac 上用 host LVGL + SDL2 验证 `app/ui/` 页面；D12x/Luban-Lite 构建时把同一份 `app/ui` 源码编进 `libep_app_core.a`，LVGL display/touch 仍由 SDK 提供。
...
```text
进入具体业务协议、业务模块和 UI 页面开发，优先从 app/services/ 与 app/ui/ 已有边界往下接
```
...
- `app/ui/`：建立可在 Mac host 和 AIC Luban-Lite 共用的 LVGL 页面入口。

后续主线建议：

1. 电源板 UART2 协议按实际帧格式单独实现。
2. 明确后续业务模块边界，例如 UI 页面流程、配置、用户数据、资源加载。
3. 业务需要蜂鸣器、RTC、LCD sleep 时直接调用当前 `app/services/` 接口，继续补产品级调用点和板级冒烟步骤。
