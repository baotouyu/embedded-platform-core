...
```text
整理并补齐 Luban-Lite 兼容层契约，再按 OSAL、HAL、设备映射逐步实现真实 port
```

KI-141103-480p 已经完成基础板级启动和镜像构建验证。下一步不急着写业务代码，应先把 OSAL、HAL、设备兼容层的参数、返回值、生命周期、错误码和冒烟测试记录清楚，再按文档逐步补真实实现。
